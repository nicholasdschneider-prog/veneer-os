...
const output = new URL("../docs/reports/decision-images/", import.meta.url)
  .pathname;
...
try { for (const width of [320,375,414,768,1080,1440,1920]) for (const theme of ['light','dark']) {
 ...
 if(p.includes('/images/')) {if(p.endsWith('/2'))return route.fulfill({status:404});if(p.endsWith('/1'))await new Promise(r=>setTimeout(r,200));return route.fulfill({contentType:'image/png',body:png});}
 ...
 assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));assert.equal(mutations,0);assert.deepEqual(errors,[]);
