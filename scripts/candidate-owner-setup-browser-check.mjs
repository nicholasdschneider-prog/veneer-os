...
const output=new URL('../docs/reports/autoship-candidate-setup/',import.meta.url).pathname;await mkdir(output,{recursive:true});
...
try {
 for(const width of [375,1440]) for(const theme of ['light','dark']) {
  ...
  await page.route('**/api/**',async route=>{
   ...
   if(p==='/api/bot-communication/autoship-candidates/setup'){
    ...
    if(route.request().method()==='POST'){
     ...
     setup={...setup,status:'registered',receipt:{source_id:'fixture-source',owner_id:1,account_id:'fixture-account',request_key:'fixture-stable-key',created_at:'2026-09-24'}};
  ...
  await page.goto('http://127.0.0.1:3297/#/autoship-candidate-setup');
  const confirm=page.getByRole('button',{name:'Confirm candidate notifications'});await confirm.waitFor();assert.equal(posts,0);
  ...
  await page.getByRole('heading',{name:'Setup confirmed'}).waitFor();assert.equal(posts,1);assert.match(await page.getByLabel('Registration receipt').inputValue(),/fixture-source/);
  ...
  await page.getByRole('alert').waitFor();assert.equal(await page.getByLabel('Registration receipt').count(),0);assert.equal(posts,2);
