...
const output=new URL('../docs/reports/routine-policy-enrollment/screenshots/',import.meta.url).pathname;await mkdir(output,{recursive:true});
try{
for(const width of [320,375,414,768,1440])for(const theme of ['light','dark']){
 ...
 await page.getByText('Standing routine authority',{exact:true}).click();await page.getByRole('button',{name:'Check routine setup'}).click();await page.getByRole('status').filter({hasText:'Routine sending is not enabled'}).waitFor();
 assert(await page.getByRole('button',{name:'Send message',exact:true}).isVisible());assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));assert.deepEqual(errors,[]);
