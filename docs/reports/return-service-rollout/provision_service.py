"""Bounded BUILD259 operator: never prints credentials or unfiltered API bodies."""
...
def main():
 ...
 if len(sys.argv)>1 and sys.argv[1]=='provision':
  ...
  domain='nicksworld.dev/api/return-exception/verifier'
  name='orderops-return-exception-verifier'
  ...
  store('OO_RETURN_VERIFIER_CF_CLIENT_SECRET',token['client_secret'])
  store('OO_RETURN_VERIFIER_CF_CLIENT_ID',token['client_id'])
  app=api('POST',base+'/apps',{'name':'Veneer Return Exception Verifier','domain':domain,'type':'self_hosted','session_duration':'0s','service_auth_401_redirect':True})
  policy=api('POST',base+'/apps/'+app['id']+'/policies',{'name':'OrderOps exact return verifier','decision':'non_identity','include':[{'service_token':{'token_id':token['id']}}]})
  store('OO_RETURN_VERIFIER_CF_AUD',app['aud'])
  store('OO_RETURN_VERIFIER_ORIGIN',origin)
  receipt={'accountId':account,'origin':origin,'applicationId':app['id'],'audience':app['aud'],'serviceTokenId':token['id'],'serviceClientId':token['client_id'],'policyId':policy['id'],'domain':domain,'secretDestination':'ervp/prd/OO_RETURN_VERIFIER_CF_CLIENT_SECRET'}
  ...
  values={'VP_RETURN_VERIFIER_CF_AUD':app['aud'],'VP_RETURN_VERIFIER_CLIENT_ID':token['client_id']}
