...
export function DecisionChoices({ choices, disabled, onChoose }: {
  ...
  return <div className="mt-4 flex flex-col gap-2">
    {(choices ?? defaultDecisionChoices).map((choice, index) => <Button key={choice.id} type="button" className="h-auto min-h-12 justify-start whitespace-normal py-3" variant="outline" disabled={disabled} onClick={() => onChoose(choice.id)}>
      ...
      <span className="min-w-0 text-left"><span className="block">{!choices && choice.action === 'approve' ? 'Approve recommendation' : choice.label}</span>{choice.description && <span className="block text-xs font-normal text-muted-foreground">{choice.description}</span>}<span className="block text-xs font-normal text-muted-foreground">{choice.action === 'approve' ? 'Approves this proposal' : choice.action === 'reject' ? 'Declines this proposal' : choice.action === 'defer' ? 'Defers this decision' : 'Withdraws this request'}</span></span>
    </Button>)}
  </div>;
}
