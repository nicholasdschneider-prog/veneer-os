...
export function VoiceCallPanel({ callerName, name, botId, status, active, connected, muted, level, history, ready, onStart, onMute, onEnd, onStandby, children }: {
  callerName?: string; name: string; botId: string; status: string; active: boolean; connected: boolean; muted: boolean;
  ...
  const initials = callerName?.trim().split(/\s+/).map(word => word[0]).slice(0,2).join('').toUpperCase() || 'You';
  ...
  return <section aria-label={`Live voice · ${name}`} className="conversation-surface flex max-h-[75dvh] flex-col overflow-hidden rounded-[2rem] border bg-background p-3 shadow-xl">
    ...
    <div className="min-h-0 overflow-y-auto overscroll-contain">
      ...
      {transcript && <ol aria-label="Saved voice conversation" className="mt-4 flex flex-col gap-5 px-2 pb-3 text-base leading-relaxed">
