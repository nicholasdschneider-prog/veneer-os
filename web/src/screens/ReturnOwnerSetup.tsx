...
type Setup = {
  ...
  receipt: { trust_id: string; owner_id: number; account_id: string; request_key: string; created_at: string } | null;
};
const endpoint = '/api/bot-communication/return-exception/setup';
...
export function ReturnOwnerSetup() {
  ...
  return <main className="mx-auto w-full max-w-2xl space-y-6 p-5 pb-16 sm:p-8">
    <header className="space-y-2"><p className="text-sm text-zinc-500">Elkhart RV Parts · Owner setup</p><h1 className="text-2xl font-semibold">Connect the prepared return service</h1><p className="text-zinc-600 dark:text-zinc-300">Confirm the connection prepared for Avery’s return-window exception workflow.</p></header>
    <section className="space-y-3 rounded-2xl border border-zinc-200 p-5 dark:border-zinc-700">
      ...
      <p>Connect Avery to the dedicated OrderOps Production return verifier, using the reviewed source account and service identity.</p>
      <p className="text-sm text-zinc-600 dark:text-zinc-300">This saves the setup only. It does not issue a label, refund, customer message or return. Your existing customer approval stays unchanged. The source operator still needs to finish the connection and acceptance checks.</p>
    ...
    {(uncertain || (!busy && !setup)) ? <button className={button} disabled={busy} onClick={() => void check()}>Check registration status</button>
      : setup?.status === 'ready' && <button className={button} disabled={busy} onClick={() => void confirm()}>Confirm return service setup</button>}
    ...
    <a className="inline-block py-3 text-sm text-blue-600 underline dark:text-blue-400" href="#/bot-guide?feature=return-owner-setup">Read the setup guide</a>
  </main>;
}
