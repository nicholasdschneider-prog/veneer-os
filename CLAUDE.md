...
same worktree. **Always commit your work**: commit every finished, verified change as soon as it is
done and push it to `origin main` right away — never leave work uncommitted at the end of a task and
never wait to be asked. Deploying (`npm run restart`) still follows the build/test loop above.
American spelling everywhere, including identifiers and UI copy.
