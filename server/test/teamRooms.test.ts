...
describe("private team rooms", () => {
  ...
  afterEach(() => db.close());
  it('shows only room worker activity and never exposes private worker details', () => {
    const room = create();
    post(room.id, owner, {text: '@bot-a plain text'});
    expect(s.read(ali, room.id).bot_activity).toEqual([]);
    post(room.id, owner, {mentions: ['bot:bot-a']});
    const worker = wakes()[0]!.conversation_id;
    const state = () => s.read(ali, room.id).bot_activity[0]!.state;
    expect(state()).toBe('queued');
    db.prepare("UPDATE conversation_wakeups SET status='delivered'").run();
    expect(state()).toBe('no_reply');
    db.prepare("INSERT INTO pending_turns(conversation_id,prompt,status) VALUES('bot-a','unrelated private work','failed')").run();
    expect(state()).toBe('no_reply');
    db.prepare("DELETE FROM pending_turns WHERE conversation_id='bot-a'").run();
    db.prepare("INSERT INTO queued_messages(conversation_id,prompt,sort_order) VALUES(?,'queued room work',0)").run(worker);
    expect(state()).toBe('queued');
    db.prepare('DELETE FROM queued_messages').run();
    db.prepare("INSERT INTO pending_turns(conversation_id,prompt) VALUES(?, 'private prompt')").run(worker);
    expect(state()).toBe('working');
    db.prepare("UPDATE pending_turns SET status='failed',error='private error'").run();
    expect(state()).toBe('failed');
    const view = JSON.stringify(s.read(ali, room.id).bot_activity);
    expect(view).not.toContain(worker);
    expect(view).not.toContain('private');
    expect(() => s.activity(other, room.id)).toThrow();
    db.prepare('DELETE FROM pending_turns').run();
    db.prepare("INSERT INTO questions(request_id,conversation_id,turn_id,response_mode,questions_json) VALUES('q',?,'turn','poll','[]')").run(worker);
    expect(state()).toBe('waiting');
    db.prepare('DELETE FROM questions').run();
    post(room.id, { ...owner, conversationId: worker }, {text: 'Room reply'});
    expect(state()).toBe('replied');
    db.prepare("UPDATE conversation_wakeups SET status='cancelled'").run();
    expect(state()).toBe('cancelled');
    db.prepare("UPDATE team_room_members SET left_at=datetime('now') WHERE room_id=? AND bot_id='bot-a'").run(room.id);
    expect(s.read(ali, room.id).bot_activity).toEqual([]);
  });
