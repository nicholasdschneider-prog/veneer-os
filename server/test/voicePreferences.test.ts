...
import { applyVoicePreferenceResult, manageVoicePreferences, readVoicePreferences, voicePreferenceToolSchema, voiceStyleInstructions } from '../src/voice/preferences.js';
...
describe('personal voice style', () => {
  it('refreshes current-call instructions on save/reset and reports refresh failures honestly', async () => {
    ...
    await applyVoicePreferenceResult({ ok: true, preferences: { length: 'concise' } }, 'Existing rules', update);
  ...
  it('persists across database reopen, isolates humans, merges updates, and resets only the caller', () => {
    ...
    try {
      manageVoicePreferences(db, 1, { action: 'update', preferences: { length: 'concise', tone: 'direct' } });
      manageVoicePreferences(db, 2, { action: 'update', preferences: { length: 'detailed' } });
      db.close(); db = new Database(file);
      expect(readVoicePreferences(db, 1)).toEqual({ length: 'concise', tone: 'direct' });
      expect(manageVoicePreferences(db, 1, { action: 'update', preferences: { structure: 'answer_first' } }).preferences)
        .toEqual({ length: 'concise', tone: 'direct', structure: 'answer_first' });
      for (const args of [
