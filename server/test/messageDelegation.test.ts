...
describe('exact approved message delegation',()=>{
 ...
 it('does not retire delegated messages through ordinary draft retirement',()=>{const {d}=setup();expect(()=>s.retire(executor,d.id,{expected_version:d.version,request_key:'retire',reason:'obsolete',evidence:'fixture'})).toThrow('delegation lifecycle');expect(s.readDraft(executor,d.id).state).toBe('queued');});
