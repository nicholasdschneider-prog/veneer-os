...
const proposal = {
  ...
  properties: {
    ...
    choices: { type: 'array', minItems: 2, description: 'Offer all relevant concise contextual buttons for a bounded human decision; there is no fixed option count cap. Every choice explicitly maps to approve, reject, defer, or withdraw. Hold/Not now must never approve. Omit for default buttons.', items: { type: 'object', properties: { id: {type:'string', pattern:'^[a-zA-Z0-9_-]{1,64}$'}, label:{type:'string', maxLength:120}, description:{type:'string',maxLength:300}, action:{type:'string',enum:['approve','reject','defer','withdraw']} }, required:['id','label','action'], additionalProperties:false } },
...
export const BOT_TOOL_DEFINITIONS = [
  ...
  definition('claim_routine_message', 'Claim an accepted routine draft once with a DIFFERENT fresh source proof of unchanged material and scope. This reserves only and returns execute=false. Only the dedicated authenticated source service may obtain a one-time dispatch-claim tied to its pre-existing durable intent; never forward bot JSON as send authority. Fresh native-context coverage and ownership/lease/hold/duplicate guards remain required. Direct legacy sends are prohibited. Lost response/unknown effect: reconcile read-only, never retry send or use a new key. Trusted source SENT readback closes delivery; record_message_delivery can only mark routine uncertainty.', {draft_id:str,proof_id:str,claim_key:str}, ['draft_id','proof_id','claim_key']),
  ...
  definition('reply_message_thread', 'Reply to an individual result’s thread, preserving its original context. Replies appear chronologically in the main chat with a link to that result. Use existing decision discussion tools for approval questions. A reaction is never approval.', {thread_id:str,request_key:str,text:str}, ['thread_id','request_key','text']),
  ...
  definition(
    'raise_decision',
    'Raise a persistent human decision for THIS registered bot. When asking for a business approval or hold, present clickable proposal.choices here instead of plain-text yes/no bullets. Does not stop unrelated work or grant authority. Dedupe uses source_key + proposal_key. Use conversation evidence references. Release external ticket/case leases with their owning system first, then record park_decision_work. Business fleet enrollment uses explicit delegated preview/apply tools.',
