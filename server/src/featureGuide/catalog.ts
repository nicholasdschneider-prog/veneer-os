...
export const BOT_FEATURES: BotFeature[] = [
  {
    ...
    summary:'Use one central human decision; keep source and authority repairs owned internally.',
    steps:['Open the bot’s outgoing message card and read its lifecycle and repair owner.', 'For Needs human decision, open the existing central decision. Technical setup blockers are not another per-email approval request.', 'Review original approved-message obligations separately from ordinary drafts. Sent requires a receipt; unknown outcomes require reconciliation.'],
    example:'Show which CS replies need a real human decision and which are waiting for technical follow-through.',
    limits:'Presentation never authorizes delivery. Existing non-CS human draft controls remain. Unbound drafts are not certified routine, and similar text does not inherit an approval. Source adapter, dedicated routine transport and genuine owner enrollment remain separate prerequisites. Legacy unscoped native decisions conservatively block routine execution.',
    agent:'Read list_message_drafts cs_lifecycle and approved_obligations as presentation only. Keep original exact approvals separate from differing ordinary drafts; never retrofit or automatically retire them. No duplicate per-email approval workaround. claim_routine_message now reserves with execute=false; only the dedicated source POST /api/routine-message/verifier/dispatch-claims can return first execute=true for its durable intent. Captures require native_context_revision from authenticated native-context; native unresolved decisions/holds deny. Lost first dispatch response is UNKNOWN; reconciliation never permits sending. Unsupported categories and legacy unstructured approval remain blocked, not autoauthorized.',
  },
  {
  ...
  },
  {
    ...
    announcement:'A guarded native path now accepts exact missing-information templates under enrolled standing policies. It requires separately authenticated source verification; no live source adapter or automatic sends were enabled by this release.',
    audience:'Authenticated business owners for policy/source enrollment; named executors for verified drafts; authorized draft readers for setup status',
    summary:'Separate policy-level authority from per-email human approval without inventing historical approvals.',
    steps:['Open an outgoing draft, expand Standing routine authority, and select Check routine setup.', 'The business owner enrolls the exact policy, source reference, business, categories and named executors through the authenticated routine-policies API documented in the release contract. Enrollment records the current owner, not a guessed historical actor.', 'The owner enrolls a separately configured and reviewed routine source adapter. Its complete-context proof supplies an exact native missing-information template; the named bot uses inspect_routine_proof, saves that exact draft, then accept_routine_message.', 'Claim requires a different fresh proof with unchanged material through claim_routine_message. Only the enrolled source dispatcher may deliver once; trusted exact SENT readback confirms delivery. Unknown results remain read-only reconciliation.'],
    ...
    limits:'Enrollment API only; no policy editor yet. All categories currently disabled in production: no routine source adapter or service trust was activated. The native contract supports only fixed missing-information templates with complete source context, actually missing relevant fields, holds, lease and duplicate/unknown-effect checks. Other categories remain unsupported; return service credentials never supply routine authority. No live policy was enrolled by the release. Bot membership or manager coordination does not grant enrollment or send authority. Financial/remedy/material exceptions retain human gates. No-contact completion is not extra-email authority.',
    agent:'Use list_routine_policies with verified business_id and inspect_routine_message with policy_id, category and exact canonical case/executor/payload scope. Both are read-only. Without a fresh trusted proof, ready=false and execute=false. Only a separately enrolled routine source may supply proof_id for the exact missing_information template; use inspect_routine_proof, save_message_draft with that exact payload, accept_routine_message, then claim_routine_message with a different fresh unchanged proof. That bot claim only reserves (execute=false). The dedicated source must obtain its own one-time dispatch-claim before execution; trusted source readback, not bot assertion, records SENT. Unknown effects require read-only reconciliation. No production adapter was enabled by this release; factual tracking and other categories remain unsupported. Only an authenticated human business owner can enroll or revoke policies. Never parse historical prose into approval, invent authorized_by, use category/eligible assertions as proof, or request duplicate per-email approval as a workaround. Preserve leases, holds, exact scope, unknown-effect reconciliation and financial gates. Guide: /#/bot-guide?feature=routine-policy-enrollment.',
  },
  {
  ...
  {
    id: 'voice', title: 'Talk with your bot', category: 'Daily work', updated: '2026-09-23', announcement: 'Use the same blue waveform to call from chat, bot cards, decision details, and voice start/resume controls. Saved calls remain at their time in the message timeline.',
    audience: 'Employees with call access', summary: 'Discuss a task out loud while staying connected to its actual chat.',
    steps: ['Tap the blue waveform in chat, a bot card, or a decision card, then Start voice. Labeled Call and Resume controls use the same waveform.', 'Allow microphone access and clearly state any instruction you want sent to the working bot.', 'Use the speech bubble to expand the saved transcript, the microphone to mute, and the red X to end. Settings includes Standby and audio guidance. The waveform reflects real audio activity and your initials identify your side of the call.', 'After a new call ends, find its Voice chat card at the time of the call among messages in the original conversation; open it to read your saved transcript and connected duration. These are private to the caller; no call audio recording is provided. Interrupted and failed calls are labeled.'],
    ...
    agent: 'Point users to the blue waveform for live calls; the microphone is dictation/mute and Listen is playback. Treat explicit dispatched voice requests as work in the existing thread. Report from actual chat evidence, distinguish queued work from completed work, and retain approval boundaries. Direct callers to their Voice chat card at the time of the call in the message timeline for the saved session transcript; do not claim it contains an audio recording or that another teammate can read their private call.',
  },
  {
