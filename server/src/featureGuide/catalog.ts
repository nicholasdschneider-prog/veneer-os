...
export const BOT_FEATURES: BotFeature[] = [
  {
    ...
    steps:['After routine source enrollment, open /#/routine-scope-review and select an existing decision.', 'Read the original decision, enter actual OrderOps case IDs and record links, then select Prepare case handoff. Refresh selected decision to read the immutable handoff and exact source lookup request.', 'Give the original OrderOps custodian the handoff reference and exact request. Its own named connection submits the existing source scope-evidence request; only the dedicated service can retrieve one exact native handoff. There is no service inventory endpoint.', 'Refresh selected decision after the source returns its complete projection. Review every related case and record either the complete case set, Unknown or Entire business. Unknown and business-wide scope keep blocking.', 'Use Revoke case handoff or Revoke scope classification when needed. After an uncertain result, refresh the selected decision to reconcile the existing record before doing anything else.'],
    example:'Prepare the exact source case lookup for this hold, then show me its full scope for review.',
    limits:'Preparation records lookup data only and does not approve a reply or narrow an obligation. The source verifies complete relationships; the current owner alone classifies the original full scope. Changed decision/event, root selection, revocation, incomplete projection or stale evidence blocks execution. No private native inventory or decision prose is exposed to the service. Source schemas/configuration, genuine owner enrollment and independent acceptance remain required; technical deployment is not activation or SENT.',
    announcement:'Owners now have a structured case handoff and scope review page. Exact source requests are versioned and revocable, and the source result remains separate from owner classification and customer-send authority.',
    agent:'Use /#/routine-scope-review only for the actual current owner after source enrollment. Preserve the exact handoff/source request UUID, roots and native decision tuple. Never infer roots from prose, prepare or classify as the owner, or fetch native inventory through a service. The original source custodian uses the exact existing scope-evidence payload and own registered identity; dedicated GET /api/routine-message/verifier/scope-handoffs/:id is exact-ID only. First observation must use the handoff request key and include all selected roots. Revoked/superseded handoffs invalidate their evidence and coverage; explicit fresh source observations do not approve scope. Keep lost-response reconciliation read-only and original business decisions unchanged.'
  },
  {
    id:'routine-owner-setup',title:'Authorize routine product-label photo requests',category:'Automation',updated:'2026-09-25',
    audience:'The current Elkhart RV Parts business owner; Platform Dev and the existing OrderOps source owner prepare setup.',
    summary:'Review one bounded standing authorization for the exact product-label-photo request, once its source connection is ready.',
    steps:['Open /#/routine-reply-setup in your own signed-in session.', 'If setup is blocked, read the named technical owner and prerequisite; no approval is needed yet.', 'Once ready, review the exact message, policy and sending identity, then select Authorize routine photo requests.', 'After an uncertain response, select Check registration status. Platform Dev uses the actual nonsecret policy/trust receipt to finish source activation.'],
    example:'Finish the photo-request connection and prepare the standing authorization for my review.',
    limits:'Dedicated routine transport is provisioned. Source deployment/runtime and current executor evidence must be reviewed before confirmation is available. Registration does not prove source activation or customer delivery. Only the fixed product-label-photo request is covered; model-number, installation-photo, tracking, financial and remedy messages are excluded. Holds, duplicate prevention and unknown-effect reconciliation remain binding.',
    announcement:'Routine photo requests now have an owner setup page with explicit prerequisites, one atomic policy/connection confirmation and a server-enforced photo-only limit. Customer sending remains gated on completed source setup and genuine owner enrollment.',
    agent:'Use /#/routine-reply-setup for the actual business owner only after technical prerequisites are complete. Never enroll as owner, guess deployment/executor evidence, reuse return credentials or treat setup as SENT. The enrolled missing_information_fields restriction is enforced at capture and later execution checks. Reconcile uncertain confirmation read-only using stable keys. Finish source activation with the original custodian and actual receipts. A retired draft does not resolve the customer request; verify independent delivery or retain the outstanding case. Avoid repeated drafting against unchanged setup blockers.'
  },
  {
  ...
  },
  {
    ...
    steps:['Open an outgoing draft, expand Standing routine authority, and select Check routine setup.', 'For the prepared product-label-photo workflow, the business owner uses /#/routine-reply-setup once its prerequisites are complete. Other reviewed policy setup uses the authenticated routine-policies API documented in the release contract. Enrollment records the current owner, not a guessed historical actor.', 'The owner enrolls a separately configured and reviewed routine source adapter. Its complete-context proof supplies an exact native missing-information template; the named bot uses inspect_routine_proof, saves that exact draft, then accept_routine_message.', 'Claim requires a different fresh proof with unchanged material through claim_routine_message. Only the enrolled source dispatcher may deliver once; trusted exact SENT readback confirms delivery. Unknown results remain read-only reconciliation.'],
  ...
  {
    id: 'handoffs', title: 'Hand work to another bot', category: 'Teamwork', updated: '2026-09-24', announcement: 'Bot exchanges now stay in separate coordination threads, with a compact View conversation link in the human chat.',
    ...
    limits: 'A handoff does not grant access or authorize customer-facing actions. Viewing a coordination thread requires access to both original chats. New messages use separate sessions; earlier history stays where it was. Work sharing a bot’s connections is serialized, with queued human work first. Delivery and acceptance are different from completion.',
    agent: 'Use send_message for one other bot, with a concrete outcome, evidence references and a stable request_key reused only for the identical message. New bot messages and replies stay in a coordination thread; they never steer an active human turn. Use read_coordination with the returned thread id for results, then summarize relevant findings in the human task. Both original chats must remain accessible. Executor identity, connections and exact approval gates remain bound to the original bot; a separate session grants no new authority. Use huddles for sustained multi-bot work and schedule a real wakeup when follow-up is needed after the turn ends.',
  },
  {
